...
namespace DBLinqProvider.Data.Common
{
    /// <summary>
    ...
    public class SkipToRowNumberRewriter : DbExpressionVisitor
    {
        ...
        protected override Expression VisitSelect(SelectExpression select)
        {
            ...
            if (select.Skip != null)
            {
                ...
                newSelect = newSelect.AddColumn(new ColumnDeclaration("o_rownum", new RowNumberExpression(select.OrderBy), colType));

                // add layer for WHERE clause that references new rownum column
                newSelect = newSelect.AddRedundantSelect(this.language, new TableAlias());
                newSelect = newSelect.RemoveColumn(newSelect.Columns.Single(c => c.Name == "o_rownum"));

                var newAlias = ((SelectExpression)newSelect.From).Alias;
                ColumnExpression rnCol = new ColumnExpression(typeof(int), colType, newAlias, "o_rownum");
                Expression where;
                if (select.Take != null)
                {
