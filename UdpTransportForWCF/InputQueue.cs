...
namespace UdpTransportForWCF
{
    ...
    class InputQueue<T> : IDisposable where T : class
    {
        ...
        //有可用项立马执行回调，无可用项则将回调封装后放入Reader队列
        ...
        {
            //Console.Out.WriteLine("BeginWaitForItem");
            lock (ThisLock)
            {
                if (queueState == QueueState.Open)
                {
        ...
        class AsyncQueueReader : AsyncResult, IQueueReader
        {
            ...
            bool expired;
            InputQueue<T> inputQueue;//将类定义在泛型类内部，则泛型类型T不需要该类重新申明，这技巧挺不错的
            T item;
            Timer timer;
                ...
                : base(callback, state)
            {
                ...
                if (timeout != TimeSpan.MaxValue)
                {
                    this.timer = new Timer(timerCallback, this, timeout, TimeSpan.FromMilliseconds(-1));//到期后将自身从队列中移除，只执行一次
                }
            }
            ...
            public void Set(Item item)
            {
                ...
                if (this.timer != null)
                {
                    this.timer.Change(-1, -1);//有用时则不再移除自己
                }
        ...
        {
            Item[] items;
            int head;//表示有效项从第几位开始
            int pendingCount;//挂起项数量，即暂不可用的项（仍属于有效项）的数量
            int totalCount;//有效项数量
            ...
            void EnqueueItemCore(Item item)
            {
                ...
                items[tail] = item;//最末有效项（不一定是数组的最末位）
                totalCount++;
            }

            //取第一位有效项，并把该索引置为默认项（单纯new了一个，为无效项）
