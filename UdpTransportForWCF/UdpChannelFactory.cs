...
namespace UdpTransportForWCF
{
    ...
    class UdpChannelFactory : ChannelFactoryBase<IOutputChannel>
    {
        ...
        protected override IOutputChannel OnCreateChannel(EndpointAddress remoteAddress, Uri via)
        {
            ...
            //if (context != null)
            //{
            //    MessageProperties properties = context.OutgoingMessageProperties;
            //    if (properties.ContainsKey("via"))
            //    {
            //        via = new Uri(properties["via"].ToString() + via.AbsolutePath);
            //        remoteAddress = new EndpointAddress(via.ToString());
            //    }
            //}
